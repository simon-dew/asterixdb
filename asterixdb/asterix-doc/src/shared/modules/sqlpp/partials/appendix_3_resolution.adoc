In this Appendix, we'll look at how variables are bound and how names
...
.  _In a FROM clause_: Names in a FROM clause identify the collections
...
quantified predicate, are as follows:
 ...
 ..  If the identifier matches a variable-name that is in scope, it
...
block; this is called a correlated subquery.)
 ..  Otherwise, if the identifier is the first part of a two-part name
...
.  _Elsewhere in a query block_: In clauses other than FROM, a name
...
block.)
 ..  (The "Single Variable Rule"): Otherwise, if the FROM clause in the
...
.  _In a standalone expression_: If a query consists of a standalone
...
.  Once the leftmost identifier has been resolved, the following dots
